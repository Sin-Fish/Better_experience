...
public class CreativeItemMoveStrategy implements ItemMoveStrategy {
    ...
    public void swapSlots(ClientPlayerEntity player, Slot slotA, Slot slotB) {
        ...
        if (client.currentScreen != null) {
            try {
                client.currentScreen.getClass().getMethod("onSlotChanged").invoke(client.currentScreen);
            } catch (Exception e) {
                LogUtil.warn("Inventory", "无法调用onSlotChanged方法: " + e.getMessage());
            }
        }
